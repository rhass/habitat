...
impl<'a> Migrator<'a> {
    ...
    pub fn setup(&self) -> Result<()> {
        self.xact.execute("SET search_path TO public", &[]).map_err(Error::SchemaSwitch)?;
        ...
        for shard in self.shards.iter() {
            ...
            schema_xact.execute("SET search_path TO public", &[]).map_err(Error::SchemaSwitch)?;
            ...
            schema_xact.execute(&set_search_path, &[]).map_err(Error::SchemaSwitch)?;
            schema_xact.execute(r#"CREATE TABLE IF NOT EXISTS builder_db_migrations (
            ...
            schema_xact.execute(&next_id_v1, &[]).map_err(Error::FunctionCreate)?;
    ...
    pub fn migrate(&mut self, prefix: &str, sql: &str) -> Result<()> {
        for shard in self.shards.iter() {
            ...
            let set_search_path = format!("SET search_path TO {}", schema_name);
            self.xact.execute(&set_search_path, &[]).map_err(Error::SchemaSwitch)?;
