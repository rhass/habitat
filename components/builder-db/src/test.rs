    ...
    ($datastore:ident) => {
        {
            ...
            use $crate::test::{init, postgres, SHARD_COUNT, INIT_TEMPLATE, TEST_COUNT};
            ...
            let pool = Pool::new(&format!("postgresql://hab@127.0.0.1/{}", db_name), 1, 300, Duration::from_secs(3600), (0..SHARD_COUNT).collect()).expect("Failed to create pool");
...
macro_rules! with_pool {
    ($pool:ident, $code:block) => {
        ...
        let $pool = Pool::new("postgresql://hab@127.0.0.1/builder_db_test", 1, 300, Duration::from_secs(3600), (0..SHARD_COUNT).collect(), true).expect("Failed to create pool");
        $code
    }
}
    ...
    () => {
         {
            ...
            postgres::start();
            init::create_database();
            Pool::new("postgresql://hab@127.0.0.1/builder_db_test", 1, 300, Duration::from_secs(3600), (0..SHARD_COUNT).collect(), true).expect("Failed to create pool")
        }
    }
}
