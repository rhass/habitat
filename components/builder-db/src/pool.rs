...
thread_local!(static HASHER: RefCell<FnvHasher> = RefCell::new(FnvHasher::default()));
...
impl Pool {
    ...
    pub fn get_shard
        ...
        conn.execute(&sql_search_path, &[]).map_err(Error::SchemaSwitch)?;
        Ok(conn)
    }
    ...
    pub fn get<T: Routable>
        (&self,
         routable: &T)
         -> Result<r2d2::PooledConnection<r2d2_postgres::PostgresConnectionManager>> {
        let optional_shard_id = routable.route_key()
            .map(|k| HASHER.with(|h| (k.hash(&mut *h.borrow_mut()))));
